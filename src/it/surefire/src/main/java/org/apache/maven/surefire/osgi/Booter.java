...
public class Booter extends ForkedBooter {
	...
	public static void _main(String[] args) throws Throwable {
		...
		try {
			...
			InputStream stream = surefirePropertiesFile.exists() ? new FileInputStream(
			...
			ProviderConfiguration providerConfiguration = booterDeserializer
					.deserialize();
			StartupConfiguration startupConfiguration = booterDeserializer
					.getProviderConfiguration();
			ClasspathConfiguration classpathConfiguration = startupConfiguration
					.getClasspathConfiguration();
			ClassLoader testClassLoader = classpathConfiguration
					.createTestClassLoader();
			Set<BundleWiring> bundleWirings = Collections.emptySet();
			{
				String symbolicName;
				Version version;
				URL resource = testClassLoader
						.getResource(JarFile.MANIFEST_NAME);
				InputStream inputStream = resource.openStream();
				try {
					Manifest manifest = new Manifest(inputStream);
					Attributes mainAttributes = manifest.getMainAttributes();
					symbolicName = mainAttributes
							.getValue(Constants.BUNDLE_SYMBOLICNAME);
					String bundleVersion = mainAttributes
				...
				for (Bundle candidateBundle : candidateBundles) {
					String bundleSymbolicName = candidateBundle
							.getSymbolicName();
					Version bundleVersion = candidateBundle.getVersion();
					if (bundleSymbolicName.equals(symbolicName)
							&& bundleVersion.equals(version)) {
						Dictionary<String, String> headers = candidateBundle
								.getHeaders();
						String fragmentHost = headers
								.get(Constants.FRAGMENT_HOST);
						if (fragmentHost == null) {
							BundleWiring bundleWiring = candidateBundle
									.adapt(BundleWiring.class);
							bundleWirings = Collections.singleton(bundleWiring);
						} else {
							BundleRevisions bundleRevisions = candidateBundle
									.adapt(BundleRevisions.class);
							if (bundleRevisions != null) {
								List<BundleRevision> candidateBundleRevisions = bundleRevisions
										.getRevisions();
								bundleWirings = new HashSet<BundleWiring>(
										candidateBundleRevisions.size());
								for (BundleRevision candidateBundleRevision : candidateBundleRevisions) {
									BundleWiring candidateBundleWiring = candidateBundleRevision
											.getWiring();
									if (candidateBundleWiring != null) {
										List<BundleWire> candidateBundleWires = candidateBundleWiring
												.getRequiredWires(null);
										if (candidateBundleWires != null) {
											for (BundleWire candidateBundleWire : candidateBundleWires) {
												BundleWiring bundleWiring = candidateBundleWire
														.getProviderWiring();
			...
			for (BundleWiring bundleWiring : bundleWirings) {
				...
				TypeEncodedValue forkedTestSet = providerConfiguration
						.getTestForFork();
				Object testSet = forkedTestSet != null ? forkedTestSet
						.getDecodedValue(classLoader) : null;
